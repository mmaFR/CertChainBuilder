...
A simple helper to build a certificate chain based on a server certificate.
```
Usage of CertChainBuilder:
  -CertificateChainPath string
    	the path to use to dump the certificate chain (default "chain.pem")
  -ServerCertificatePath string
    	the certificate path to use to build the certificate chain (default "server.pem")
  -i string
    	the certificate path to use to build the certificate chain (default "server.pem")
  -o string
    	the path to use to dump the certificate chain (default "chain.pem")
...
# Example
```
$ CertChainBuilder -i github-com.pem
```
This command will create the chain.pem file with the certificate chain as content.\
chain.pem content :
```
-----BEGIN CERTIFICATE-----
